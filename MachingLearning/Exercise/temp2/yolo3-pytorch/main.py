...
def f(me):
    ...
    if(img_chosen<0 or img_chosen>len(file_dir)):
        ...
        #image_jpg=Image.open(source[img_chosen]).resize((600,600))
        image_jpg=Image.open('img_res/result.jpg').resize((600,600))
...
def temp():
    global c
    if(img_chosen==len(file_dir)-1):
        c=video.vid(top,img)
    if c<0 or c>19:
        image2=Image.open('null.png').resize((100,100))
        c=19
    else:
        image2=Image.open('VOCdevkit/VOC2007/JPEGImages/obj%d'%(c+1)+'__1.jpg')
